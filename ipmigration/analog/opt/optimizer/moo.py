...
from ipmigration.analog.opt.ml import ml_predictor
...
class MyProblem(Problem):
    # Define Problem
    def __init__(self, n_var, n_obj, n_ieq_constr, xl, xu, simulation, spec, objective_index, targets_type, vtype=int):
...
class Optimizer:
    def __init__(self, cfg): 
        ...
        #simulationFun_dict, targets, boundary, population_size, num_offsprings, seed=None, objective_index=[]):
        
        # cfg.    
        self.optimizer_type = cfg.optimizer_type
        self.boundary = cfg.boundary
        self.targets  = cfg.targets
        self.bounds   = self.boundary.get_normalized_variables_range()
        self.num_design_variables=len(self.bounds)
        self.spec=cfg.targets['all targets value']
        self.targets_type=cfg.targets['min_max_type']
        self.simulationFun_dict=cfg.hierarchy_sim_dict
        self.simulationFun=cfg.hierarchy_sim_dict["all"]
        self.population_size=min(cfg.population_size, 5*self.num_design_variables)
        self.num_offsprings=min(cfg.num_offsprings, cfg.population_size) # used in nsga2
        self.seed=cfg.seed
        self.objective_index=cfg.objective_index #list
        self.n_obj=None
    ...
    def _create_algorithm(self):
        ...
        if self.optimizer_type=="NSGA2":
            return self._create_algorithm_NSGA2()
        elif self.optimizer_type=="RVEA":
            return self._create_algorithm_REVA()
        elif self.optimizer_type=="USGA3":
            return self._create_algorithm_USGA3()
        ...
        #                            seed, objective_index)
            ...
            raise ValueError('%s optimizer is not supported!'%(self.optimizer_type))
 
    
    
    def _create_algorithm_USGA3(self):
    ...
    def _create_algorithm_NSGA2(self):
        population_size=self.population_size
        num_offsprings=self.num_offsprings
        #n_obj=self.n_obj
    ...
    def _create_algorithm_REVA(self):
        population_size=self.population_size
        #num_offsprings=self.num_offsprings
        n_obj=self.n_obj
        # create the reference directions to be used for the optimization
