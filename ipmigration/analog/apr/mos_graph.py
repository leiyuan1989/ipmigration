...
from api_skill import extract_layout_data
...
def gen_graph(layout_data: Dict[str, List[Dict]], include_edge_touch: bool = False) -> nx.Graph:
	"""
...
def get_chains(G):
    ...
    for endpoint in endpoints:
        if endpoint in processed_nodes:
            continue
    ...
    # 按x坐标对每条链的节点进行排序（从左到右）
    sorted_chains = []
    for chain in chains:
        # 按节点的x属性排序
        sorted_chain = sorted(chain, key=lambda node: G.nodes[node]['x']) #maybe condider Y in future
...
if __name__ == "__main__":
	...
	parser.add_argument(
		"-i", "--input", default="data/layout_data.txt", help="Path to layout data text file"
	)
	parser.add_argument(
	...
	G = gen_graph(layout_data, include_edge_touch=args.include_edge_touch)
