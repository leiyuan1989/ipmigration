#convert data to skill functions
...
dbCreatePRBoundary?no need temporary
...
def extract_layout_data(file_path: str) -> Dict[str, List[Dict]]:
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if len(parts) >= 11 and parts[1] == 'pmos3v':
                data["pmos"].append({
                    "Comp_Name": parts[0],
                    "Type": parts[1],
                    "LLX": float(parts[2]),
            ...
            elif len(parts) >= 11 and parts[1] == 'nmos3v':
                data["nmos"].append({
                    "Comp_Name": parts[0],
                    "Type": parts[1],
                    "LLX": float(parts[2]),
            ...
            elif len(parts) >= 11 and parts[1] == 'mimcap':
                data["mimcap"].append({
                    "Comp_Name": parts[0],
                    "Type": parts[1],
                    "LLX": float(parts[2]),
            ...
            elif len(parts) >= 12  and parts[1] == 'rnhpoly':
                data["rnhpoly"].append({
                    "Comp_Name": parts[0],
                    "Type": parts[1],
                    "LLX": float(parts[2]),
...
def extract_layout_mos_master(layout_data,tech):
    master_dict = {}
    inst_dict = {}
    res_dict = {}
    cap_dict = {}
    #TODO, need consider no R0 condition
    for mos in layout_data['pmos']:
        l = float(mos['Length'][:-1])
        w = float(mos['Width'][:-1])
        r = mos['Rotate']
        t = 'PMOS'
        inst_dict[mos['Comp_Name']] = [t,l,w,r] 
        if r == 'R0':
            if (t,w,l) not in master_dict:

                box = Box([mos['LLX']-mos['X'],
                           mos['LLY']-mos['Y'],
                           mos['URX']-mos['X'],
                           mos['URY']-mos['Y'] ])
                # print('test1',box)
                master_dict[(t,w,l)] = Mos('master',t,tech,w,l,box)
    for mos in layout_data['nmos']:
        l = float(mos['Length'][:-1])
        w = float(mos['Width'][:-1])
        r = mos['Rotate']
        t = 'NMOS'
    ...
    for res in layout_data['rnhpoly']:
        l = float(res['Length'][:-1])
        w = float(res['Width'][:-1])
        r = res['Rotate']
        s = int(res['Segments'])
        t = 'RES'
        inst_dict[res['Comp_Name']] = [t,w,l,r,s] 

    for cap in layout_data['mimcap']:
        l = float(cap['Length'][:-1])
        w = float(cap['Width'][:-1])
        r = cap['Rotate']
        t = 'CAP'
        inst_dict[cap['Comp_Name']] = [t,w,l,r] 

                        
    return master_dict,inst_dict
...
if __name__ == "__main__":
    ...
    INPUT_FILE = "data/layout_data.txt"       # Path to your layout data file
    OUTPUT_PLOT = "data/non_vias_layout.png"  # Path to save the final plot
